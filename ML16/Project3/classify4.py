# super hiararchial classification based on the whole image histograms
...
def load_full(mode, targets, parent_data_folder, cube_size, bins):
    X = []
    y = []
    X_file = join(parent_data_folder, 'X_full_'+str(bins)+'.npy')
    y_file = join(parent_data_folder, 'y_full_'+str(bins)+'.npy')
    X_test_file = join(parent_data_folder, 'X_full_'+str(bins)+'_test.npy')
    folder_seg = None
    folder_seg_train = join(parent_data_folder, 'set_train')
    folder_seg_test = join(parent_data_folder, 'set_test')
    if mode == 'train':
        folder_seg = folder_seg_train
    elif mode == 'test':
        folder_seg = folder_seg_test
    files = [f for f in os.listdir(folder_seg) if isfile(join(folder_seg, f)) and f.endswith('.nii')]
    sort_nicely(files)    
    ...
    for f in files:
        ...
        if n % 20 == 0:
            print 'Iteration ('+mode+') ', n

        x = nib.load(join(folder_seg, f)).get_data()
        x = np.sum(x, axis=3)       # sum upon the 4th axis so that we reduce the img to 3d
        
        # spatial histograms
        x = spatial_hist(x, cube_size, bins)
...
if isfile(X_file) and isfile(y_file) and isfile(X_test_file):
    ...
    X, y = load_full('train', mapped_targets, parent_data_folder, cube_size, bins)
    X_test = load_full('test', mapped_targets, parent_data_folder, cube_size, bins)   
...
anova = SelectKBest(f_classif, k=12000)
